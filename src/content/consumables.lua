-- src/content/consumables.lua
-- Data-driven consumable loading system

local ContentLoader = require("src.core.content_loader")

-- Load base game consumables
local baseConsumables = require("data.consumables_base")
ContentLoader.loadConsumables(baseConsumables)

-- Note: All consumables are now loaded from data/consumables_base.lua

local consumables_unused = {
    removal_token = {
    ...
    lucky_charm = {
        ...
        color = {1, 0.9, 0.3},
        on_use = function(self, engine)
            -- Next spin guaranteed good result (add temp bonus)
    ...
    symbol_copy = {
        ...
        color = {0.7, 0.5, 1},
        on_use = function(self, engine)
            -- Copy a random symbol in inventory
...
-- Removed: for k, v in pairs(consumables) do Registry.registerConsumable(k, v) end
